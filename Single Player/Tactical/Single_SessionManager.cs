...
public class Single_SessionManager : MonoBehaviour {
	...
	void Start () {
		...
		//Instantiate non-networked  linker
	...
	void Update(){
		...
		if (!hasSent && toStartCount) {
			if (isTimeUp) {
				...
				linker.GetComponent<Single_Linker>().Send();
